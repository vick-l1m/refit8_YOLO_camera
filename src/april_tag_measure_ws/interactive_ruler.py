...
from measurements import compute_px_per_meter_from_tag, solve_tag_pose, intersect_ray_with_tag_plane
...
class InteractiveRuler:
    ...
    def redraw(self, preview_line: Optional[Tuple[Tuple[int, int], Tuple[int, int]]] = None) -> None:
        ...
        status = f"Tag {self.tag_det['id']} | backend={self.backend} | mode={mode} | tag={self.tag_size_m:.4f} m"
